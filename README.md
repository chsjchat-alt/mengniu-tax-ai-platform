...
> 本仓库为「蒙牛AI创新赛」参赛项目迭代版。方案总纲见 [`docs/蒙牛AI创新_方案_迭代版V4.md`](docs/蒙牛AI创新_方案_迭代版V4.md)，
> 开源组件选型/集成/踩坑记录见 [`docs/OPEN_SOURCE_INTEGRATION.md`](docs/OPEN_SOURCE_INTEGRATION.md)。
> 核心机制：Rule ID 法规治理矩阵（六级优先级 + 生效窗口 + 复核闸门）、乳业核定扣除引擎（全 Decimal 金额运算）、
> 四流稽核（Jaccard + rapidfuzz 双分数对照）、审计证据链（calc_id 确定性指纹 + 参数快照）。
